...
public class Player : MonoBehaviour {
    ...
    protected PlayerInput playerAction; //角色动作
    ...
    protected void Start()
    {
        ...
        playerAni.ChangeWeaponModel("2Hand-Sword", MainHandWeaponType.TwoHandSword, null, OffHandWeaponType.Empty);
    ...
    protected bool noDirInput = true;
    bool lastNoDirInput = true;
    protected float inputYaw = 0;
    void Simulate()
    {
        if (grounded)
        {   //在地上
            if (!inAction) //不在特殊动作中
            {
                ...
                {
                    playerAction.roll = false;
                    this.orientation = inputYaw;
                ...
                else if (playerAction.jump)
                {
                    ...
                    if (!noDirInput)
                    {
                        this.orientation = inputYaw; //设置角色方向
                        //设置水平速度
